...
def diagnostics():
    """Safe-to-expose snapshot of what the app can see in this environment -
    never returns actual secret values, only whether they're present and
    which underlying env var name supplied them. Useful for debugging
    deploy-time config issues without digging through raw logs."""
    def env_status(names):
        ...
        for key in os.environ:
            key_lower = key.lower()
            for name in names:
                if key_lower.endswith(name.lower()) and key != name:
                    return {"found": True, "via": key}
        return {"found": False, "via": None}

    return jsonify({
        "on_vercel": bool(os.environ.get("VERCEL")),
        "database": env_status(["DATABASE_URL", "POSTGRES_URL"]),
        "blob_storage": env_status(["BLOB_READ_WRITE_TOKEN"]),
        "cron_secret_set": bool(current_app.config.get("CRON_SECRET")),
    })
