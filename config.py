...
def _int_env(name: str, default: int) -> int:
    """os.environ.get's default only kicks in when the var is truly unset -
    if Vercel (or anyone) sets it to an empty string, get() returns '' and
    int('') crashes. Treat blank the same as unset."""
...
class Config:
    ...
    STORAGE_DIR = os.environ.get("STORAGE_DIR") or os.path.join(BASE_DIR, "storage")
    ...
    CLEANUP_INTERVAL_MINUTES = _int_env("CLEANUP_INTERVAL_MINUTES", 15)
    ...
    RATELIMIT_UPLOAD = os.environ.get("RATELIMIT_UPLOAD") or "10 per hour"
